...
def visualize_autoencoder_results(original_image, slots, attention_maps, autoencoder, 
    ...
    with pt.no_grad():
        ...
        # Merge attentions (sum, NO normalization - keep same scale as other attentions)
        H, W = attention1.shape
        merged_attention_np = attention1 + attention2
        
        # Debug: merge되는 slot들 중 하나가 최대값인 픽셀 찾기
        max_pos1 = np.unravel_index(attention1.argmax(), attention1.shape)
        max_pos2 = np.unravel_index(attention2.argmax(), attention2.shape)
        
        # slot 0이 최대인 픽셀에서 비교
        test_y, test_x = max_pos1
        print(f"\n[DEBUG] Slot {idx1}이 최대인 픽셀 ({test_y}, {test_x}) 에서:")
        print(f"  Merge 전:")
        for i in range(num_slots):
            print(f"    Slot {i}: {attention_maps[i][test_y, test_x]:.6f}")
        print(f"  Merge 후 (Remaining + Merged):")
        for i in remaining_indices:
            print(f"    Slot {i}: {attention_maps[i][test_y, test_x]:.6f}")
        print(f"    Merged ({idx1}+{idx2}): {merged_attention_np[test_y, test_x]:.6f}")
        
        # slot 1이 최대인 픽셀에서 비교
        test_y2, test_x2 = max_pos2
        print(f"\n[DEBUG] Slot {idx2}이 최대인 픽셀 ({test_y2}, {test_x2}) 에서:")
        print(f"  Merge 전:")
        for i in range(num_slots):
            print(f"    Slot {i}: {attention_maps[i][test_y2, test_x2]:.6f}")
        print(f"  Merge 후 (Remaining + Merged):")
        for i in remaining_indices:
            print(f"    Slot {i}: {attention_maps[i][test_y2, test_x2]:.6f}")
        print(f"    Merged ({idx1}+{idx2}): {merged_attention_np[test_y2, test_x2]:.6f}\n")
    ...
    print(f"  Merged attention array shape: {merged_attention_all.shape}")
    print(f"  Max per slot in merged array: {[merged_attention_all[i].max() for i in range(len(merged_attention_all))]}")
    ...
    attention_to_split = attention_maps[decoder_idx]  # (256, 256)
    
    with pt.no_grad():
        slot_recon1, slot_recon2 = autoencoder.decode(slot_to_split)
        
        # Split attention using autoencoder
        # Downsample to 16x16 (training size)
        attention_16 = cv2.resize(attention_to_split, (16, 16), interpolation=cv2.INTER_LINEAR)
        attention_flat = pt.from_numpy(attention_16).flatten().unsqueeze(0).to(device)  # (1, 16*16)
        attention_split1, attention_split2 = autoencoder.decode_attention(attention_flat)
        
        # Reshape to 16x16 and upsample to 256x256
        attention_split1_16 = attention_split1.squeeze(0).cpu().numpy().reshape(16, 16)
        attention_split2_16 = attention_split2.squeeze(0).cpu().numpy().reshape(16, 16)
        
        attention_split1_np = cv2.resize(attention_split1_16, (256, 256), interpolation=cv2.INTER_LINEAR)
        attention_split2_np = cv2.resize(attention_split2_16, (256, 256), interpolation=cv2.INTER_LINEAR)
...
if __name__ == "__main__":
    ...
    model_path = f"/home/jaey00ns/MetaSlot-main/slotae/pth/linear/20260326_040316/final.pth"
